...
// ZMIANA 1: Zaktualizowane importy
import { useFormStatus } from 'react-dom'; // useFormStatus nadal jest w react-dom
import { useActionState as useActionStateFromReact, useEffect, useRef } from 'react'; // Importujemy useActionState z react
...
export default function UpdatePasswordForm() {
  // ZMIANA 2: Używamy nowej nazwy hooka
  const [state, formAction] = useActionStateFromReact(updateUserPassword, initialState);
  ...
  return (
    <form ref={formRef} action={formAction} className="mt-6 space-y-4 max-w-sm">
      {/* ... reszta formularza (pola input) pozostaje bez zmian ... */}
      <div>
        <label
          htmlFor="password"
